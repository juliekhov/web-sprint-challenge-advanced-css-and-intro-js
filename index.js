...
// cannot use .splice to replace information, array cannot be changed with splice

console.log(artists[8].name = "Van Gogh");
...
function getArtistByIndex(artists, index) {
    // cycle through array elements with for-in loop
    for (let i in artists) {
        // if statement to check if current counter variable matches index parameter
        if (i == index) {
...
function get20s(artists) {
    let cent20 = [];
    for (let i in artists) {
        // var to hold access to array years key
        let artistYrs = artists[i].years;
        // converts string to number for birthYr and deathYr
        let birthYr = parseInt(artistYrs.slice(0, 4));
        let deathYr = parseInt(artistYrs.slice(7));
        // if-statement to check if integer falls between 1900 and 2000
        // if deathYr is before 2000, then birthYr cannot be after 2000
        if (birthYr >= 1900 && deathYr <= 2000) {
            // use push method to store artist names into new array cent20
            cent20.push(artists[i].name);
        }
    }
    // return new array with names of artists
    return cent20;
}
...
function removeArtist(artists, index) {
    // use splice method to select index and remove artist from array
...
function addArtist(artists) {
    // create new object containing artist bio
    const julieObj = {
            id: artists.length,
            name: "Julie Khov",
            years: "1994 - Present",
            genre: "Web Design",
            nationality: "Asian",
            bio: "Loves to learn and writer for personal tech blog. Also enjoys listening to lofi music and studying Python on Fridays."
        }
        // push object
    artists.push(julieObj);
    return artists;
}
...
function lotsOfArt(artists) {
    let oneHundredPaintings = [];
    for (let index in artists) {
        if (artists[index].paintings > 100) {
            oneHundredPaintings.push(artists[index].name);
        }
    }
